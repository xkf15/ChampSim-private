#!/bin/bash
#SBATCH --job-name=al_t_ld_kx      # create a short name for your job
...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_alderlake_ld
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_alderlake_tage_ld.out
