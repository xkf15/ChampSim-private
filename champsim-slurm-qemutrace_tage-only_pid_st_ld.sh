...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage-only_detailed_misses_pid_st
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${TRACE_NAME}_tageonly_pid.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_store_states_tageonly_pid.out
sbatch champsim-slurm-qemutrace_tage-only_pid_ld.sh $1 $2 $3
