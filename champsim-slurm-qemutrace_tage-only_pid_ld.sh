#!/bin/bash
#SBATCH --job-name=Load_misses      # create a short name for your job
...
STOP_THRESHOLD=$3
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage-only_detailed_misses_pid_ld
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${1}_tageonly_pid.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} -p ${STOP_THRESHOLD} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_load_states_prefetchstats_tageonly_pid_threshold${3}.out
