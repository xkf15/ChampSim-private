...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage-only-replay-lessmemory_detailed_misses_st
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${TRACE_NAME}_tageonly_lessmemory.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_store_states_tageonly_lessmemory.out
sbatch champsim-slurm-qemutrace_tage-only-replay-lessmemory_detailed_missess_ld.sh $1 $2
