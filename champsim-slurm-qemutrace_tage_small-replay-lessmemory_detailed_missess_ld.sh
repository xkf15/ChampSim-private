...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage_small-replay-lessmemory_detailed_misses_ld
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${1}_tagesmall_lessmemory.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_load_states_tagesmall_lessmemory.out
