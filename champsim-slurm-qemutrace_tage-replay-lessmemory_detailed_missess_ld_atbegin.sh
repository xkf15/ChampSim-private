#!/bin/bash
#SBATCH --job-name=LD_Misses      # create a short name for your job
...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage-replay-lessmemory_detailed_misses_ld_atbegin
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_load_states_atbegin_lessmemory.out
