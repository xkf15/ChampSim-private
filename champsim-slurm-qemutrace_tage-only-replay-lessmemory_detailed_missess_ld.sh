...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage-only-replay-lessmemory_detailed_misses_ld
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${1}_tageonly_lessmemory.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_load_states_tageonly_lessmemory.out
