...
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage-replay_detailed_misses_st
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${TRACE_NAME}_morememory.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_store_states_morememory.out
sbatch champsim-slurm-qemutrace_tage-replay_detailed_missess_ld.sh $1 $2
