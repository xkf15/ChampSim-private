...
# $1 trace name for storing bp states
# $2 trace name for loading bp states
TRACE_NAME=$1 # nokvm-2023-11-13_-_23-41-55-chameleon.trace
BIN=/tigress/kaifengx/ChampSim_qemu/ChampSim-private/bin/champsim_tage_small-replay-lessmemory_detailed_misses_st
BP_STATES=/scratch/gpfs/kaifengx/tage-replay_states/${TRACE_NAME}_tagesmall_lessmemory.states
...
$BIN --warmup_instructions ${WARMUP_INSN} --simulation_instructions ${SIM_INSN} -c -s ${BP_STATES} ${TRACE}  > ${OUTPUT_DIR}${TRACE_NAME}_store_states_tagesmall_lessmemory.out
sbatch champsim-slurm-qemutrace_tage_small-replay-lessmemory_detailed_missess_ld.sh $1 $2
