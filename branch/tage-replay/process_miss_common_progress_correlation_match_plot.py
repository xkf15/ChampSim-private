...
for delta in range(1):
    print("Iteration: ", delta, flush=True)
    delta_matched = []
    ...
    for his_len in [128]: # , 256, 512, 1024]:
        ...
        for bench_l in bench_labels:
            ...
            for fn in fnames:
                    ...
                    for line in f_misses:
                        ...
                        if bench_round == 1:
                            ...
                            if tokens[3] == 'M' and (pc, phist) in br_pc:
                                common_miss += 1
                                delta_matched.append(br_pc[(pc, phist)]["line_cnt"] - line_cnt)
                                print(br_pc[(pc, phist)]["line_cnt"])
                            if tokens[3] == 'M':
                                total_miss += 1
                            if len(ghist) < his_len:
                                ghist += tokens[4]
                ...
                print("Total Branch:", line_cnt, flush=True)
                print("Unique PC and History", unique_pc_his, "Tuple Matched", same_pc_his, "Diff than prev(Reload won't help):", diff_than_prev, "Same PC and History but misses", same_pc_his_miss, flush=True)
                print("Branch misses:", total_miss, flush=True)
            
                print(bench_l, "Common misses:", common_miss, common_miss/total_miss, flush=True)
                if bench_round > 1:
    ...
    print("Progressive Results", common_miss_all_list, flush=True)
    common_miss_corr_list.append(statistics.mean(common_miss_all_list[0]))
    print("Average 128: ", statistics.mean(common_miss_all_list[0]), flush=True)
    ...
    delta_matched_list.append(statistics.mean(delta_matched))
    print("Average delta: ", statistics.mean(delta_matched), flush=True)
...
# with open("miss_common_progressive_corr_delta.csv", "w") as f_output:
#     store_list(f_output, common_miss_corr_list)
# print(common_miss_corr_list, flush=True)
# print(delta_matched_list, flush=True)
