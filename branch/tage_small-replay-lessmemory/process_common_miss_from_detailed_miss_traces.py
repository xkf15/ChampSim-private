...
for fn in fnames:
    with open(fn, "r") as f_br:
        ...
        fn_o = "miss_trace_real" + str(f_cnt) + ".txt"
...
for i in range(len(fnames)):
    preprocess_all_common_miss("miss_trace_real" + str(i) + ".txt")
...
for i in range(len(fnames)):
    ...
    correct = 0
    if_matched = False
    with open("miss_trace_real" + str(i) + ".txt", "r") as f_miss_trace:
        for line in f_miss_trace:
            tokens = line.split()
            if len(tokens) < 3:
                continue
...
def write2file():
    ...
    f_trace_o = "pc_his_common_miss_real_trace.txt"
