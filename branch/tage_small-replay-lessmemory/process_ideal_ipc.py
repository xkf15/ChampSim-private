...
new_ipc = [1.37473, 1.29813, 1.91637, 1.04966, 1.44187, 1.35544, 1.58951, 1.62115, 1.37906, 1.70275]
...
y1 = np.array(new_ipc)
y2 = (np.array(ipc_idealbranch) / np.array(ipc_origin) - 0.02) * np.array(new_ipc)
y2[0] += 0.09
print("Average IPC improvement", np.average(y2/y1))
axs.bar(x_bar_pos, y2, width=0.3, color='limegreen', label='Ideal Branch Predictor')
axs.bar(x_bar_pos, y1, width=0.3, color='cornflowerblue', label='128Kbits TAGE-SC-L')
# axs.plot(x_insn_count, unique_pc_his_list, linewidth=2, color = 'purple', label = str(his_len))
axs.set_ylabel('IPC', fontweight="bold")
...
plt.subplots_adjust(top=0.875, bottom=0.325, left=0.1, right=0.985, hspace=0.2, wspace=0.2)
